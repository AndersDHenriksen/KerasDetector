...
from dl_tools.nn.ConvTransform1DOut import ConvTransform, CenterOfMass
...
from dl_tools.utils.freeze_tools import save_frozen_protobuf2, augment_for_ocv
from dl_tools.utils.tensorboardtools import tensorboard_launch, TensorBoardImage
...
def train():
    ...
    # load data
    data_gen, validation_data = data_generator.get_data(config)

    # load model or create new
    if config.load_model:
        center_of_mass = CenterOfMass(config.input_shape[1])
    ...
    tensorboard_image = TensorBoardImage(model, config.log_dir, validation_data)
    callbacks = [model_checkpoint, tensorboard, tensorboard_image]
    ...
    if config.save_final_model:
        ...
        save_frozen_protobuf2(config.checkpoint_dir + "final_model.pb", K.get_session(),
